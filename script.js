...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const cardsData = [
    {
      // Display words with a slash to avoid revealing the correct answer immediately
      text: 'пів/Києва',
      type: 'half',
      explanation:
  ...
  function updateGeneralProgress() {
    ...
    // Query progress elements dynamically because they appear only on the finish screen
    const fill = document.querySelector('.general-progress-fill');
    const text = document.querySelector('.general-progress-text');
    if (fill) {
      fill.style.width = `${percent}%`;
    }
    if (text) {
      text.innerHTML = `${generalCompleted} / 9`;
    }
  }
  ...
  function makeDraggable(card) {
    ...
    const pointerUpHandler = (event) => {
      ...
      if (droppedOnShelf) {
        ...
        // Proceed to next card after a longer delay to allow reading the explanation
        currentIndex++;
        setTimeout(() => {
          showNextCard();
        }, 2500);
